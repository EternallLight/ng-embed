(function () {
    ...
    angular.module('ngEmbed', ['ngSanitize'])
        ...
        .constant('NG_EMBED_NOEMOJI_TAGS', ['pre', 'blockquote'])
    ...
    EmbedFilter.$inject = ['$sce', 'NG_EMBED_BASIC_EMOTICONS', 'NG_EMBED_EMOJI_LIST', 'NG_EMBED_NOEMOJI_TAGS', 'NG_EMBED_REGEXP_PATTERNS', 'NG_EMBED_FILTER_DEFAULT_OPTIONS'];
    function EmbedFilter($sce, NG_EMBED_BASIC_EMOTICONS, NG_EMBED_EMOJI_LIST, NG_EMBED_NOEMOJI_TAGS, NG_EMBED_REGEXP_PATTERNS, NG_EMBED_FILTER_DEFAULT_OPTIONS) {

        var EMOIJ_REGEX = new RegExp(":(" + NG_EMBED_EMOJI_LIST.join("|") + "):", "gi");
        var NOEMOJI_CONTENT_REGEX = new RegExp(
            '(' + NG_EMBED_NOEMOJI_TAGS.map(function(tag) {
                return '<' + tag + '>[\\S\\s]*?(<\\/' + tag + '>)';
            } ).join('|') + ')', 'gi'
        );
        ...
        return function (input, userOptions) {
            ...
            if (options.emoji) {
                input = insertEmoji(input, EMOIJ_REGEX, NOEMOJI_CONTENT_REGEX);
            }
     ...
     * @param  {RegExp}  noEmojiContentPattern
     *
     * @return {string}
     */

    function insertEmoji(str, emojiPattern, noEmojiContentPattern) {
        var noEmojiMatch = str.match(noEmojiContentPattern);
        if (!noEmojiMatch) {
        ...
        var output = str;
        var validPieces = [];
        noEmojiMatch.forEach(function(piece, i) {
            var pieceStartPos = output.indexOf(piece);
            var pieceEndPos = pieceStartPos + piece.length;
            if (pieceStartPos > currentPosition) {
                var validPiece = output.substring(currentPosition, pieceStartPos);
                validPieces.push(validPiece);
            }
            if (noEmojiMatch.length === i + 1) {
                var lastValidPiece = output.substring(pieceEndPos);
                validPieces.push(lastValidPiece);
            }
            currentPosition = pieceEndPos;
        });

        validPieces.forEach(function(validPiece) {
            output = output.replace(validPiece, validPiece.replace(emojiPattern, function (match, text) {
                return "<i class='emoticon emoticon-" + text + "' title=':" + text + ":'></i>";
            }));
        });
